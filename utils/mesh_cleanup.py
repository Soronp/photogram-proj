...
def run(run_root: Path, project_root: Path, force: bool, logger):
    ...
    try:
        ...
        logger.error(f"[mesh_cleanup] FAILED")
        raise FileNotFoundError(
            f"Missing config.yaml in run directory ({run_root}). "
            f"Ensure you created a run using create_runtime_config."
        ) from e
    ...
    if mesh_out.exists() and not force:
        logger.info("Cleaned mesh already exists — skipping")
        return
    ...
    min_area_ratio = float(mesh_cfg.get("min_component_area_ratio", 0.01))
    ...
    # Optional decimation
    # --------------------------------------------------
    decimation_ratio: Optional[float] = mesh_cfg.get("decimation_ratio")
    ...
    report = {
        ...
        "smoothing_iterations": mesh_cfg.get("smoothing_iterations", None),
    ...
    logger.info("[mesh_cleanup] COMPLETED successfully")
...
def main():
    ...
    parser.add_argument("run_root", type=Path, help="Run root containing config.yaml")
    parser.add_argument("project_root", type=Path, help="Project root containing mesh/")
    parser.add_argument("--force", action="store_true")
    args = parser.parse_args()

    logger = get_logger("mesh_cleanup", args.project_root)
    run(args.run_root, args.project_root, args.force, logger)
