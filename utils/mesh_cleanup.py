...
- Uses surface-area-aware connected component filtering
...
# ------------------------------------------------------------------
def run_mesh_cleanup(project_root: Path, force: bool):
    project_root = project_root.resolve()
    paths = ProjectPaths(project_root)
    config = load_config(project_root)
    logger = get_logger("mesh_cleanup", project_root)
    ...
    logger.info("=== Mesh Cleanup Utility ===")
    ...
    logger.info(
        f"Main component: cluster {main_cluster} "
        f"(area={main_area:.3f})"
    )
    ...
    for tri_idx, cluster_id in enumerate(tri_clusters):
        area = cluster_areas[cluster_id]

        if keep_largest and cluster_id != main_cluster:
            if area < main_area * min_area_ratio:
                remove_tris.append(tri_idx)

    logger.info(
        f"Removing {len(remove_tris):,} triangles "
        f"from floating components"
    )
    ...
    if decimation_ratio is not None:
        ...
        if 0.0 < ratio < 1.0:
            ...
            logger.info(
                f"Applying decimation "
                f"(ratio={ratio}, target_tris={target:,})"
            )
...
def main():
    ...
    run_mesh_cleanup(args.project_root, args.force)
