...
# ------------------------------------------------------------------
def run_command(cmd: list, logger, label: str):
...
# ------------------------------------------------------------------
# Check for existing sparse model
# ------------------------------------------------------------------
def find_sparse_model(sparse_root: Path) -> Optional[Path]:
    ...
    for subdir in sorted(sparse_root.iterdir()):
        if subdir.is_dir() and required_files.issubset({f.name for f in subdir.iterdir()}):
            return subdir
    return None


# ------------------------------------------------------------------
# Dataset analysis using SQLite
# ------------------------------------------------------------------
def analyze_dataset(db_path: Path, logger) -> Dict:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        cursor.execute("SELECT COUNT(*) FROM matches WHERE LENGTH(data) > 100")
        good_match_count = cursor.fetchone()[0]

    expected_pairs = (image_count * (image_count - 1)) // 2
    match_coverage = (match_pair_count / expected_pairs * 100) if expected_pairs > 0 else 0
    good_match_ratio = (good_match_count / match_pair_count * 100) if match_pair_count > 0 else 0
    ...
    logger.info(f"Match quality     : {quality}")

    return {"image_count": image_count, "match_quality": quality}
...
def select_strategy(analysis: Dict, logger) -> Dict:
    ...
    if image_count <= 30 and match_quality in {"good", "excellent"}:
        logger.info("Strategy: GLOMAP (small, high-quality dataset)")
    ...
    if match_quality == "poor":
        params += ["--Mapper.init_min_tri_angle", "1", "--Mapper.min_num_matches", "8"]
    elif match_quality == "fair":
        params += ["--Mapper.init_min_tri_angle", "2", "--Mapper.min_num_matches", "10"]
    else:
        params += ["--Mapper.init_min_tri_angle", "4", "--Mapper.min_num_matches", "15"]

    params += ["--Mapper.multiple_models", "0", "--Mapper.max_num_models", "1"]
    return {"use_glomap": False, "colmap_params": params}
...
def run_sparse_reconstruction(project_root: Path, force: bool):
    ...
    existing_model = find_sparse_model(sparse_root)
    if existing_model and not force:
        logger.info("Sparse model exists - skipping reconstruction")
        return
...
def _run_glomap(db_path: Path, images_dir: Path, sparse_root: Path, logger):
    try:
        run_command([GLOMAP_EXE, "mapper", "--database_path", db_path,
                     "--image_path", images_dir, "--output_path", sparse_root],
                    logger, "GLOMAP Mapper")
...
def _run_colmap(db_path: Path, images_dir: Path, sparse_root: Path, params: list, logger):
    cmd = [COLMAP_EXE, "mapper", "--database_path", db_path,
           "--image_path", images_dir, "--output_path", sparse_root] + params
...
def _convert_to_ply(model_dir: Path, sparse_root: Path, logger):
    ...
    run_command([COLMAP_EXE, "model_converter",
                 "--input_path", model_dir, "--output_path", ply_path, "--output_type", "PLY"],
                logger, "Sparse PLY Conversion")
